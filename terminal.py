...
class Console(urwid.Frame):
    ...
    def __init__(self, title="Title", command_caption='(Tab to switch focus to upper frame, where you can scroll text)',
                 max_size=1000):
        ...
        self.eloop = None
        self.process = pexpect.spawn(self.initConsole(), timeout=32767, encoding='utf-8')
        self.process.sendline("clear")
        self._stopped = False

    def readTillCurrent(self):  # BUG: some lines repeat as they are still on screen and not only log.
        self.output("Reading from log!")
        with open("logs/latest.log") as file:
            old = file.readlines()
        for l in old:
            self.output(l.replace("\t", "   ")[:-1])
    ...
    def initConsole(self):
        ...
        if (os.path.exists("terminal.conf")):
            ...
            print(
                "No command provided for starting server. Specify the shell command to start the server either as a command line argument or in a file 'terminal.conf'\nExiting!")
            quit()
...
console = Console(title='Terminal Client', max_size=32767,
                  command_caption="Tab to switch focus to server log.(To scroll text)")  # init class


def read(child):
    global reader
    console.output("Reading Console Output!", "CLIENT")
    console.readTillCurrent()
    while True:
