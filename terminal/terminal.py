...
def setpath():
    ...
    if os.path.exists("terminal.conf"):
        ...
        print(
            "No command provided for starting server. Specify the shell command to start the server either as a "
            "command line argument or in a file 'terminal.conf'\nExiting!")
        quit()
...
class Console(urwid.Frame):
    ...
    def __init__(self, title="Title", command_caption='(Tab to switch focus to upper frame, where you can scroll text)',
        ...
        self.eloop = None
        self.process = pexpect.spawn(setpath(), encoding='utf-8')
        self._stopped = False
